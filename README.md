...
The components of a Docker image include:
1. Base Image: It is the starting point for creating a Docker image. It can be an official image from Docker Hub or a custom image created by the user.

2. Layers: Docker images are built in layers, where each layer represents a set of changes made to the image. Each layer is immutable and can be reused across different images.

3. Metadata: Docker images also contain metadata that describes the image, such as its name, version, and author.
...
Docker Engine is the core component of Docker that enables the creation, management, and running of Docker containers. It is a client-server application that consists of three main components:

1. Docker Daemon: The Docker Daemon (also known as dockerd) is a background service that runs on the host machine and manages Docker containers. It listens for Docker API requests and manages Docker objects such as images, containers, networks, and volumes.

2. Docker Client: The Docker Client is a command-line interface (CLI) that allows users to interact with the Docker Daemon. It sends commands to the Docker Daemon using the Docker API.

3. REST API: The Docker REST API is a set of HTTP endpoints that allow developers to interact with the Docker Daemon programmatically. It provides a way to automate Docker operations and integrate Docker with other tools and services.

<!-- Docker engine image saved in the same directory with name Docker_Engine.png -->
![Docker_Engine](docker_engine.png)


## DOCKER VOLUMES
Docker Volumes are a way to persist data generated by and used by Docker containers. They are stored outside the container's filesystem, allowing data to be shared between containers and to persist even when the container is deleted. Volumes are managed by Docker and can be easily backed up, restored, and migrated.

To create and use a Docker volume, you can follow these steps:
1. Create a Docker volume using the following command:
   ```docker volume create volume_name```

2. When running a Docker container, you can mount the volume to a specific directory inside the container using the `-v` or `--mount` flag. For example:
   ```docker run -d -v volume_name:/path/in/container --name container_name image_name:tag```

    Here, `volume_name` is the name of the Docker volume, `/path/in/container` is the directory inside the container where the volume will be mounted, `container_name` is the name of the container, and `image_name:tag` specifies the image to use for the container.

3. To list all Docker volumes, you can use the following command:
   ```docker volume ls```

4. To inspect a specific Docker volume, you can use the following command:
   ```docker volume inspect volume_name```

5. To remove a Docker volume, you can use the following command:
   ```docker volume rm volume_name```

### THERE ARE OTHER TOPICS TO COVER LIKE DOCKER NETWORKS, DOCKER COMPOSE, DOCKER SWARM, ETC. WHICH WILL SHOULD BE COVERED BY READING OFFICIAL DOCKER DOCUMENTATION AND OTHER TUTORIALS.